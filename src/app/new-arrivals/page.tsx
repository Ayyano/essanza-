...
export default function NewArrivalsPage() {
  ...
  const categoryProducts = useMemo(() => {
    if (!selectedCategory) return newProducts;
  ...
  const availableCategories = useMemo(() => {
    const cats = [...new Set(newProducts.map(p => p.category))];
  ...
  return (
    <div className="min-h-screen bg-warm-white">
      <section className="relative py-20 sm:py-28 bg-gradient-to-b from-matte-black to-deep-charcoal overflow-hidden">
        <div className="absolute inset-0 opacity-[0.08]" style={{ background: 'radial-gradient(ellipse at top left, #C4A97D 0%, transparent 60%)' }} />
        <div className="relative z-10 px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto text-center">
          ...
          >
            <motion.div
              ...
              className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-600/20 border border-emerald-600/30 mb-6"
            >
              <Sparkles className="h-3.5 w-3.5 text-emerald-400" />
              <span className="text-xs text-emerald-300 font-medium uppercase tracking-wider">Fresh Arrivals</span>
            </motion.div>
            ...
            <p className="text-base sm:text-lg text-gray-400 italic font-light mt-3 max-w-xl mx-auto">
              Naya fashion, naya andaaz — sab se pehle ESSANZA par
            </p>
      ...
      <nav className="px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto py-4 flex items-center gap-2 text-xs text-gray-400 border-b border-gray-100">
        ...
        <span className="text-muted-gold">New Arrivals</span>
      </nav>
      ...
      <div className="px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto py-8">
        ...
        >
          <span className="font-medium text-matte-black">{filteredProducts.length}</span> New Products
        </motion.p>

        <ProductGrid
          products={filteredProducts}
          emptyMessage="Koi new arrivals nahi"
          emptyDescription="Jald hi naye products a rahe hain"
        />
      </div>
    </div>
  );
