...
export default function ShopPage() {
  ...
  const hasActiveFilters = selectedCategories.length > 0 || selectedSizes.length > 0 || selectedColors.length > 0 || priceRange[0] > MIN_PRICE || priceRange[1] < MAX_PRICE;
  ...
  const filterSidebar = (
    <div className="space-y-8">
      <div>
        <h3 className="text-xs uppercase tracking-wider text-gray-400 font-medium mb-4">Categories</h3>
          ...
          {categories.map(cat => (
            <label key={cat.id} className="flex items-center gap-3 cursor-pointer group">
              <input
                ...
                className="h-4 w-4 rounded border-gray-300 text-muted-gold focus:ring-muted-gold accent-muted-gold"
              />
              <span className="text-sm text-gray-600 group-hover:text-matte-black transition-colors">{cat.name}</span>
              <span className="text-xs text-gray-300 ml-auto">{cat.itemCount}</span>
            </label>
          ))}
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-warm-white">
        ...
        <div className="relative z-10 px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto text-center">
          ...
          >
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-heading font-bold text-warm-white tracking-tight">
              Shop <span className="text-muted-gold">ESSANZA</span>
            </h1>
            <p className="text-base sm:text-lg text-gray-400 italic font-light mt-3 max-w-xl mx-auto">
              Har cheez jo aap ki life ko stylish aur easy bana de
            </p>
      ...
      <div className="sticky top-0 z-20 bg-warm-white/90 backdrop-blur-md border-b border-gray-100">
        <div className="px-6 sm:px-12 lg:px-20 max-w-7xl mx-auto py-3 overflow-x-auto scrollbar-hide">
          <div className="flex items-center gap-2">
            <span className="flex-shrink-0 px-4 py-1.5 text-xs font-medium rounded-full bg-matte-black text-warm-white">
              All
            </span>
            {categories.map(cat => (
              <Link
                key={cat.id}
                href={`/shop/${cat.slug}`}
                className="flex-shrink-0 whitespace-nowrap px-4 py-1.5 text-xs font-medium rounded-full bg-soft-beige text-gray-600 hover:bg-matte-black hover:text-warm-white transition-all duration-200"
              >
                {cat.name}
              </Link>
            ))}
              ...
              {hasActiveFilters && (
                ...
                >
                  ...
                  {selectedCategories.map(cat => (
                    <span key={cat} className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-soft-beige text-gray-700">
                      {cat}
                      <button onClick={() => removeFilter('category', cat)} className="hover:text-red-500 transition-colors">
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ))}
                  {selectedSizes.map(size => (
                    <span key={size} className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-soft-beige text-gray-700">
                      {size}
                      <button onClick={() => removeFilter('size', size)} className="hover:text-red-500 transition-colors">
                        <X className="h-3 w-3" />
                      </button>
                    </span>
                  ))}
                  {selectedColors.map(color => {
                    const c = allColorEntries.find(co => co.name === color);
                    return (
                      <span key={color} className="inline-flex items-center gap-1.5 px-3 py-1 text-xs font-medium rounded-full bg-soft-beige text-gray-700">
                        ...
                        {color}
                        <button onClick={() => removeFilter('color', color)} className="hover:text-red-500 transition-colors">
                          <X className="h-3 w-3" />
                        </button>
                      </span>
                    );
